...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/16 19:48:20 by kyabuuch          #+#    #+#             */
/*   Updated: 2025/04/10 16:02:11 by kyabuuch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	set_env(t_env *env, char **args, char ***path)
{
	...
	if (!*args)
		return (0);
	if (!**args)
		return (err_cmd_str_msg("export", args[0], ERR_IDF), \
		set_env(env, args + 1, path), 1);
	else if (*args[0] == '=' || no_alpha_name(*args) || spchar_in_name(*args))
		return (err_cmd_str_msg("export", args[0], ERR_IDF), \
		set_env(env, args + 1, path), 1);
